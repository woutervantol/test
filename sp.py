"""
VERSION 4 Feb 2017
Compatible with python3 and Numpy 1.12 - Alex Pietrow
"""
...
REVISION HISTORY:
	...
	Made compatible with numpy 1.12 - A.G.M. Pietrow February 2017
...
INPUTS:
     ...
     skyrad -- Two element vector giving the inner and outer radiiaper
 ...
 REVISON HISTORY:
       ...
       made compatible with python3 A.G.M. Pietrow April 2016
       Made compatible with numpy 1.12 A.G.M. Pietrow February 2017
        """
	xc=numpy.array(xc)
	yc=numpy.array(yc)
...
#  Compute the limits of the submatrix.   Do all stars in vector notation.
	...
	for i in numpy.arange(0,Nstars):
		...
		if skipstar == 0:
			...
			for ii in numpy.arange( int(ny[i])):
				
				rsq[ii,:] = (dxsq) + ((ii-dy[i])**2)
                                                
			if exact == True:
				nbox = numpy.arange(nx[i]*ny[i])
				XX = (numpy.reshape(numpy.mod(nbox,nx[i]),[int(ny[i]),int(nx[i])]))
				YY = numpy.trunc(numpy.reshape((nbox/nx[i]),[int(ny[i]),int(nx[i])]))
			...
			if numpy.size(setskyval) == 0:
                                noskyval=1
                                skypix=numpy.zeros([int(ny[i]),int(nx[i])],dtype=bool)
                                for ii in numpy.arange(nx[i]):
                                        for jj in numpy.arange(ny[i]):
                                                skypix[int(jj),int(ii)] = numpy.where((rsq[int(jj),int(ii)] >= rinsq) & (rsq[int(jj),int(ii)] <= routsq))
			...
			if skipstar == 0:
                                for k in numpy.arange(Naper):
                                        if ( edge[0,i] >= apr[k] ):
                                                if exact == True:
                                                        mask = numpy.zeros([int(ny[i]),int(nx[i])])
                                                        good=numpy.zeros([int(ny[i]),int(nx[i])],dtype=bool)
                                                        bad=numpy.zeros([int(ny[i]),int(nx[i])],dtype=bool)
                                                        for ii in numpy.arange(int(nx[i])):
                                                                for jj in numpy.arange(int(ny[i])):
                                                        ...
                                                        if Nfract > 0:
                                                                ...
                                                                mask=numpy.reshape(mask,[int(ny[i]),int(nx[i])])
                                                        thisap = (mask > 0.0)
...
def mmm(sky_vector,skymod,sigma,skew,highbad=[],debug=False,readnoise=[],maxiter=50.,minsky=20.,integer=False,silent=False):
    ...
    #while(redo == 1):
        ...
        if done == False:
            done = numpy.where((sky[numpy.max(numpy.array([int(newmin),0]))] < cut1) & (tst_min))
        if done == False:
...
def Oneside(x, y0, y1, r):
    ...
    if size_x <= 1:
        ...
        t1 = (y0[ti] <= -yh[ti])
        count=numpy.sum(t1)
        t1 = numpy.arange(size_x)[numpy.where((y0[ti] <= -yh[ti]))]
        if count != 0:
            i = ti[t1]
            
            t2=(y1[i] <= -yh[i])
            count=numpy.sum(t2)
            t2= numpy.arange(size_x)[numpy.where((y1[i] <= -yh[i]))]
            if count != 0:
                j = ti[ t1[ t2 ] ]
                ans[j] =  arc( x[j], y0[j], y1[j], r )
            t2=( y1[i] > -yh[i] ) & ( y1[i] <=  yh[i] )
            count = numpy.sum(t2)
            t2 =  numpy.arange(size_x)[numpy.where(( y1[i] > -yh[i] ) & ( y1[i] <=  yh[i] ))]
            if count != 0:
                j = ti[ t1[ t2 ] ]
        ...
        t1 =  numpy.where(( y0[ti] > -yh[ti] ) & ( y0[ti] < yh[ti] ))
        count=numpy.sum(t1)
        t1=numpy.arange(size_x)[t1]
        
        if count != 0:
            i = ti[ t1 ]
            t2 = numpy.where(( y1[i] <= -yh[i]))
            count=numpy.sum(t2)
            t2=numpy.arange(size_x)[t2]
            if count != 0:
                j = ti[ t1[ t2 ] ]
                ans[j] = chord( x[j], y0[j], -yh[j] ) + arc( x[j], -yh[j], y1[j], r )
            t2 = numpy.where(( ( y1[i] > -yh[i] ) & ( y1[i] <=  yh[i] )))
            count=numpy.sum(t2)
            t2=numpy.arange(size_x)[t2]
            if count != 0:
                j = ti[ t1[ t2 ] ]
...
def saveFITS(data, filename, overwrite=False):
	...
	"""
	pyfits.writeto(filename, data, header=None, clobber=overwrite)
